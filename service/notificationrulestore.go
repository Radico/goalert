...
// services that use schedule-based alerting. Services in any other urgency have
// no rules by construction, so they are skipped rather than queried for.
func loadNotificationRules(ctx context.Context, q *gadb.Queries, svcs []Service) error {
	byID := make(map[uuid.UUID]*Service)
	for i := range svcs {
		if svcs[i].NotificationUrgency != UrgencyScheduled {
			continue
		}
...
// replaceNotificationRules makes the stored rule set match svc exactly. Normalize
// clears the rules for every urgency other than UrgencyScheduled, so this also
// cleans up after a service that leaves schedule-based alerting.
