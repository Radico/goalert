...
func MapConfigValues(cfg config.Config) []ConfigValue {
	return []ConfigValue{
		...
		{ID: "General.DisableOnCallServiceAlerts", Type: ConfigTypeBoolean, Description: "If set, the alert list will not include alerts from services a user is on-call for, leaving only favorited services, alerts that notified them, and alerts assigned to them.", Value: fmt.Sprintf("%t", cfg.General.DisableOnCallServiceAlerts)},
...
func ApplyConfigValues(cfg config.Config, vals []ConfigValueInput) (config.Config, error) {
	...
	for _, v := range vals {
		...
		case "General.DisableOnCallServiceAlerts":
			val, err := parseBool(v.ID, v.Value)
			if err != nil {
				return cfg, err
			}
			cfg.General.DisableOnCallServiceAlerts = val
