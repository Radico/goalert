...
describe('alertScheduleInput', () => {
  it('sends nothing for a service that is not scheduled', () => {
    expect(alertScheduleInput('high', 'UTC', [ruleAt('09:00', 'UTC')])).toEqual(
      {},
    )
    expect(alertScheduleInput('low', 'UTC', [ruleAt('09:00', 'UTC')])).toEqual(
      {},
    )
  })

  it('sends the zone and windows for a scheduled service', () => {
    const input = alertScheduleInput('scheduled', 'UTC', [
      ruleAt('09:00', 'UTC'),
    ])
