		...
		newPolicies: p.P(`
			with to_escalate as (
				select alert_id, step.id ep_step_id, step.delay, step.skip_if_empty, step.escalation_policy_id, a.service_id,
					ep.step_count > 1 has_later_step
				...
				join services s on a.service_id = s.id and
					s.maintenance_expires_at isnull and
					s.notification_urgency != 'low' and
					not s.notification_suppressed
