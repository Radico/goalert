...
WHERE
    svc.notification_urgency = 'scheduled';

-- name: EscMgrSetSuppressedServices :exec
-- Materializes the current alerting-window state. @open_service_ids is the set
-- of scheduled services whose window is open right now; everything else in
-- 'scheduled' mode is suppressed, and every other service is not.
--
...
UPDATE
    services
SET
    notification_suppressed = (notification_urgency = 'scheduled'
        AND NOT (id = ANY (@open_service_ids::uuid[])))
WHERE (notification_urgency = 'scheduled'
    OR notification_suppressed)
AND notification_suppressed IS DISTINCT FROM (notification_urgency = 'scheduled'
    AND NOT (id = ANY (@open_service_ids::uuid[])));
