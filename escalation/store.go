...
func NewStore(ctx context.Context, db *sql.DB, cfg Config) (*Store, error) {
	...
	return &Store{
		...
		findOneStepForUpdate: p.P(`SELECT id, escalation_policy_id, delay, step_number, skip_if_empty FROM escalation_policy_steps WHERE id = $1 FOR UPDATE`),
		findAllSteps:         p.P(`SELECT id, escalation_policy_id, delay, step_number, skip_if_empty FROM escalation_policy_steps WHERE escalation_policy_id = $1 ORDER BY step_number`),
		findAllOnCallSteps: p.P(`
			SELECT step.id, step.escalation_policy_id, step.delay, step.step_number, step.skip_if_empty
		...
		createStep: p.P(`
			INSERT INTO escalation_policy_steps
				(id, escalation_policy_id, delay, step_number, skip_if_empty)
...
func (s *Store) FindOneStepForUpdateTx(ctx context.Context, tx *sql.Tx, id string) (*Step, error) {
	...
	row := stmt.QueryRowContext(ctx, id)
	var st Step
	err = row.Scan(&st.ID, &st.PolicyID, &st.DelayMinutes, &st.StepNumber, &st.SkipIfEmpty)
	if err != nil {
		return nil, err
	}
...
func (s *Store) FindAllOnCallStepsForUserTx(ctx context.Context, tx *sql.Tx, userID string) ([]Step, error) {
	...
	for rows.Next() {
		var s Step
		err = rows.Scan(&s.ID, &s.PolicyID, &s.DelayMinutes, &s.StepNumber, &s.SkipIfEmpty)
		if err != nil {
			return nil, err
		}
...
func (s *Store) CreateStepTx(ctx context.Context, tx *sql.Tx, st *Step) (*Step, error) {
	...
	err = stmt.QueryRowContext(ctx, n.ID, n.PolicyID, n.DelayMinutes, n.SkipIfEmpty).Scan(&n.StepNumber)
	if err != nil {
		return nil, err
	}
