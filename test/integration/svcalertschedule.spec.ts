...
// The zone select is by accessible name rather than the shared dropdownSelect
// helper, which matches on an ancestor div and is ambiguous inside the service
// form (several fields share a container). It retries the whole open-and-pick
// because the control only exists once useExpFlag's query resolves.
async function enableAlertSchedule(page: Page): Promise<void> {
  await expect(async () => {
    await page
      .getByRole('checkbox', { name: 'Only alert during scheduled windows' })
      .check({ timeout: 3000 })
...
// Enabling alerting windows reveals the weekly window editor, and the
// configuration survives a round trip through the API.
test('configure a weekly alerting window', async ({ page, isMobile }) => {
  ...
  await enableAlertSchedule(page)

  // Ticking the box seeds one window, so the editor is immediately usable.
