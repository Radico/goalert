...
values
	({{uuid "open"}}, {{uuid "eid"}}, 'open service', 'scheduled', 'UTC'),
	({{uuid "closed"}}, {{uuid "eid"}}, 'closed service', 'scheduled', 'UTC');
...
func TestServiceAlertScheduleFlagGate(t *testing.T) {
	...
	require.NotEmpty(t, res.Errors, "setting scheduled urgency without the flag should be rejected")
...
// stay editable, including being switched back to a plain urgency. Otherwise
// turning the flag off strands them.
func TestServiceAlertScheduleEditableWithoutFlag(t *testing.T) {
	...
	rename := fmt.Sprintf(`mutation { updateService(input: {id: "%s", name: "renamed service"}) }`, h.UUID("open"))
	require.Empty(t, h.GraphQLQuery2(rename).Errors, "renaming a scheduled service should work without the flag")

	// And there is a way out of scheduled mode.
	toHigh := fmt.Sprintf(`mutation { updateService(input: {id: "%s", notificationUrgency: high}) }`, h.UUID("open"))
	require.Empty(t, h.GraphQLQuery2(toHigh).Errors, "leaving scheduled urgency should work without the flag")

	d := h.Twilio(t).Device(h.Phone("1"))
	h.CreateAlert(h.UUID("open"), "now high urgency")
	d.ExpectSMS("now high urgency")
}
